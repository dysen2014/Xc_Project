...
//							"http://weixin.qq.com/"));
		...
		case R.id.btn_login:
//			getLogin();
			getLogin(et_usertel.getText().toString().trim(), et_password.getText().toString().trim());
			break;
		default:
			break;
		}
...
//		getLoadingDialog("正在登录...").show();
		if (TextUtils.isEmpty(userName) || TextUtils.isEmpty(password)) {
			List<tLogin> listLogin = db.findAll(tLogin.class);
			if (listLogin.size() > 0) {
				int i = listLogin.size()-1;//最后一条
				userName = listLogin.get(i).getName();
				password = listLogin.get(i).getPwd();
				et_usertel.setText(listLogin.get(i).getName());
				et_password.setText(listLogin.get(i).getPwd());
...
//		getLogin(userName, password);
	...
	private void getLogin(final String userName, final String password) {
		if (!TextUtils.isEmpty(userName) && !TextUtils.isEmpty(password)) {
			...
			/**	2016-5-21修改*/
			Utils.putValue(LoginActivity.this, Constants.UserInfo, "");
...
//					.loadAllConversations();
			...
			Intent intent = new Intent(LoginActivity.this,
					MainActivity.class);
			Utils.showLongToast(LoginActivity.this, "登录成功！");
//			t_login.setName(userName);
//			t_login.setPwd(password);
//			db.save(t_login);
